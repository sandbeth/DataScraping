---
title: "MGT4050 Business Analytics"
author: "Karthik Babu Nattamai Kannan"
date: '`r Sys.Date()`'
output:
  word_document:
    toc: no
...
# Goals, Guidelines and Hints
  ...
  2. You will have to do string manipulation in this HW. I suggest using the package `stringr`. Please look at the help and vignette to learn and do by yourself. Learning a new R package by yourself is an important skill that needs to be developed. I am hoping this HW will train you in this aspect.
  3. You will be tested in the usage of serveral R packages including ineq, rvest, tidyverse (dplyr, stringr etc.). If you need clarifications, look at the R Help or cheatsheets.
  4. Discussion within group is allowed. However, you cannot copy the final code from others. You can discuss concepts and ask for clarifications on other examples. I would also need you to note down the team member with whom you discussed and brief nature of discussions.
  5. Due date: 13th Feb 2017 @ 11:30 PM
  6. What to submit:
      + Make a zip with the name `GTid_hw1.zip
      + Zip should contain an .Rmd file (you can just modify this one with your output), your processed datasets in .Rdata format. The .Rmd file should have working code snippets that should be executable in any other machine just by changing the base directory.
      + Use the `knit` option to convert your .Rmd file to a word document. This word document should have my questions, your explanations, code snippets, and output of your analysis. I will primarily grade your work based on the word document.
      + For more help in formatting your .Rmd file, look at (R Markdown)[http://rmarkdown.rstudio.com/]. This is the one of the latest and greatest output format used in Data Science.

# Questions
1. [50 points] Scrape data from (slickdeals)[https://slickdeals.net/]
...
2.  [50 points] Now, you will work with 2 datasets `Census_by_zip.csv` and `us_postal_codes.csv`. The former contains demographic details of each zip code. The latter contains the state, county, and lat/long for each zip. Note that in both cases Zip is stored as a number. Now, do the following
